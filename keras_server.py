...
from sqlalchemy import Engine, create_engine, engine_from_config
from sqlalchemy.orm import sessionmaker
import tensorflow as tf
from tabledef import User
from utility import img_to_encoding
...
# for checking whether a face is there in the image or not
...
def face_present(image_path):
    ...
    for x, y, w, h in faces:
        ...
        cv2.rectangle(img, (x - 10, y - 70), (x + w + 20, y + h + 40), (15, 175, 61), 4)
...
def load_FRmodel():
    try:
        model = load_model(
            "models/model.h5", custom_objects={"triplet_loss": triplet_loss}
        )
...
def face_recognition(encoding, database, model, threshold=0.6):
    ...
    authenticate = False
    # loop over all the recorded encodings in the database
...
def authenticate_user():
    ...
    query = s.query(User).filter(
        User.username.in_([POST_USERNAME]), User.password.in_([POST_PASSWORD])
    )
    result = query.first()
...
# to add user through the sign-up form
    ...
    if flask.request.method == "POST":
        ...
        if POST_USERNAME not in user_db.keys():
            ...
            if flask.request.files.get("image"):
                ...
                cv2.imwrite(
                    "saved_image/new.jpg", cv2.cvtColor(image, cv2.COLOR_RGB2BGR)
                )
            ...
            # only create a new session if complete user details are present
            if data["face_present"]:
                # create a new session
                Session = sessionmaker(bind=engine)
                s = Session()
                # add data to the user_db dict
